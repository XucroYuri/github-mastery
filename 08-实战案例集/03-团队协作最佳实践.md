...
> [!NOTE]
> 本指南假设你已经熟悉 GitHub 的基本操作（仓库管理、分支、提交），以及 [PR 完整生命周期](../02-协作与工作流/03-PR-完整生命周期.md) 和 [代码审查](../02-协作与工作流/04-代码审查.md) 的核心概念。这里重点讲解的是如何将这些能力系统化地整合到团队工作流中。
...
分支策略是团队协作的基石。参考 [分支策略与Git-Flow](../02-协作与工作流/05-分支策略与Git-Flow.md) 中的详细分析，这里给出针对不同团队规模的推荐方案。
...
手动审查容易遗漏问题，自动化检查是代码质量的第一道防线。结合 [分支保护与规则集](../04-代码质量与安全/04-分支保护与规则集.md) 中的保护规则，以下是推荐的自动化检查配置。
...
2. **PR 描述必须关联 Issue** —— 使用 `closes #xxx` 或 `fixes #xxx`，参考 [Issue 完整指南](../02-协作与工作流/01-Issue-完整指南.md) 中的自动关闭机制。
...
参考 [项目管理看板](../02-协作与工作流/06-项目管理看板.md)，为每个迭代创建看板：
...
参考 [Discussions 社区](../05-文档与知识管理/03-Discussions社区.md)，在仓库中启用 Discussions 用于：
...
- 使用 [PR 完整生命周期](../02-协作与工作流/03-PR-完整生命周期.md) 中提到的 Draft PR 功能，明确标识开发状态。
