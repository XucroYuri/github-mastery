...
你可以参考 [协作与工作流](../02-协作与工作流/README.md) 章节来了解 PR 工作流的最佳实践。
...
- **Require code scanning results**——要求 [CodeQL 扫描](01-代码扫描与Code-QL.md) 无高危告警。
...
> [!WARNING]
> 如果你的项目使用自动化发布流程
> （如通过 [Actions](../03-自动化与CI-CD/01-GitHub-Actions-入门.md) 创建 Release 并打 Tag），
...
也可以使用 [Actions](../03-自动化与CI-CD/01-GitHub-Actions-入门.md) 的 `pull-request` 触发器避免直接推送。
